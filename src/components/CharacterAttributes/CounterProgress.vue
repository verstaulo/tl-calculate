...
let progressValue = computed(
  () =>
    `${props.attributeCount * (circleLength.value / 100)} ${circleLength.value}`
);
</script>
<template>
  <svg
  ...
  >
    ...
    <circle
      ...
      stroke-width="10"
      :stroke="
        props.attributeCount >= 30 ? 'var(--secondary)' : 'var(--neutral6)'
      "
      stroke-linecap="round"
      :stroke-dasharray="`0 ${circleLength}`"
      :stroke-dashoffset="(circleLength / 100) * 70"
    ></circle>
    ...
    <circle
      ...
      stroke-linecap="round"
      :stroke-dasharray="`${0} ${circleLength}`"
      :stroke-dashoffset="`${(circleLength / 100) * 60}`"
    ></circle>
    <circle
      class="checkPoint"
      ...
      stroke-linecap="round"
      :stroke-dasharray="`${0} ${circleLength}`"
      :stroke-dashoffset="`${(circleLength / 100) * 50}`"
    ></circle>
    <circle
      class="checkPoint"
      ...
      stroke-linecap="round"
      :stroke-dasharray="`${0} ${circleLength}`"
      :stroke-dashoffset="`${(circleLength / 100) * 40}`"
    ></circle>
    <text
      x="50"
      y="60"
